...
.verboseScatterplotVecAgainstColumns = function (mat, vec, mfrow, labels=NULL, xlab=NULL, ...) {
...
.draw_pairwise_venn_from_groups = function (gp1, gp2, groupNames=NULL, ...) {
  ...
  draw.pairwise.venn(area1=length(gp1), area2=length(gp2), cross.area=length(gp1_2), 
                     category=groupNames,
...
.draw_quad_venn_from_groups = function (gp1, gp2, gp3, gp4, groupNames=NULL, ...) {
  ...
  draw.quad.venn(area1=length(gp1), area2=length(gp2), area3=length(gp3), area4=length(gp4),
                 n12=length(gp1_2), n13=length(gp1_3), n14=length(gp1_4),
                 n23=length(gp2_3), n24=length(gp2_4),
                 n34=length(gp3_4),
                 n123=length(gp1_2_3), n124=length(gp1_2_4), n134=length(gp1_3_4),
                 n234=length(gp2_3_4),
                 n1234=length(gp1_2_3_4),
                 category=groupNames,
...
.draw_triple_venn_from_groups = function (gp1, gp2, gp3, groupNames=NULL, ...) {
  ...
  draw.triple.venn(area1=length(gp1), area2=length(gp2), area3=length(gp3), 
                 n12=length(gp1_2), n23=length(gp2_3), 
                 n13=length(gp1_3),
                 n123=length(gp1_2_3),
                 category=groupNames,
...
.addDmrOVInfoToExpression = function (expr, dmrsOVbyGene, dmrsOVsimple, alldmrIDs) {
  alldmrgenes = unique(unlist(lapply(dmrsOVbyGene, names)));
  dmrcols = c('dmr.num', 'dmrfc', 'dmrfc.abs',
              'n', 'width.dmr', 'invdensity', 
              'areaStat', 'areaStat.abs', 
              'group1.mean', 'group2.mean', 'all.mean',
              'meanDiff', 'meanDiff.abs', 'dmrGC');
  ...
  for (g in alldmrgenes) {
    ...
    gdmr = subset(do.call('rbind', gdmr), 
                  !duplicated(paste0(chr,start,end)));#print(gdmr)
    #
    ...
    expr$group1.mean[row] = mean(gdmr$group1.mean);
    expr$group2.mean[row] = mean(gdmr$group2.mean);
    expr$all.mean[row] = mean(c(gdmr$group1.mean, gdmr$group2.mean));
    expr$meanDiff[row] = mean(gdmr$meanDiff);
    expr$meanDiff.abs[row] = mean(abs(gdmr$meanDiff));
    expr$dmrGC[row] = mean(gdmr$GC);
  }
  ...
  tmp = unlist(dmrsOVsimple); # appends number to dmr names
                              #   if a single dmr covers same feature in >1 genes
  tmp = split(tmp,tmp);
  dmrsOVsimpleByGene = lapply(lapply(tmp, 
                                     function(f) strsplit(names(f),'.',fixed=T)), 
                              function(ff) as.data.frame(do.call(rbind,ff)));
  for (i in 1:length(dmrsOVsimpleByGene)) {
    names(dmrsOVsimpleByGene[[i]]) = c('feature','dmr');
    dmrsOVsimpleByGene[[i]] = dmrsOVsimpleByGene[[i]][,2:1]
  }
  # fix dmr names
  checkdmrids = sapply(dmrsOVsimpleByGene, 
                       function(f) !all(f$dmr %in% alldmrIDs)) 
  for (i in which(checkdmrids)) {
    badids = !(dmrsOVsimpleByGene[[i]]$dmr %in% alldmrIDs);
    dmrsOVsimpleByGene[[i]]$dmr[badids] = substring(dmrsOVsimpleByGene[[i]]$dmr[badids],
                                                    1, 
                                                    (nchar(dmrsOVsimpleByGene[[i]]$dmr[badids])-1))
  }
  
  expr = as.data.frame(cbind(expr, dmrID=NA, dmrOVotherGene=NA));
  for (i in which(expr$dmr.num > 0)) {#print('-------------');print(i);
    thisgene = rownames(expr)[i];
    thisgenedmrs = unique(dmrsOVsimpleByGene[[thisgene]]$dmr);
    expr$dmrID[i] = paste0(thisgenedmrs, collapse=',')
    
    ogenecheck = unlist(lapply(dmrsOVsimple, function(f) f[thisgenedmrs]));
    ogenecheck = ogenecheck[ogenecheck!=thisgene];
    
    if (length(ogenecheck)>0){
      #print(thisgene);
      # print(ogenecheck);
      ogenecheck = lapply(split(ogenecheck,ogenecheck), 
                          function(f) strsplit(names(f), '.',fixed=T));
      toadd = c();
      for (g in 1:length(ogenecheck)) {
        thisothergene = names(ogenecheck)[g];
        ofeats = paste0(sapply(ogenecheck[[g]], function(f) f[1]),collapse=',')
        toadd = c(toadd, paste0(thisothergene, '(', ofeats,')'));
        
      }
      expr$dmrOVotherGene[i] = paste0(toadd,collapse=',')
...
.make_numOV = function (dmrsGRhitsOV, features, prefix='dmrsGR_', dmrsints) {
...
.boxplot2 = function (num, fact=NULL, main=NULL, names=NULL, col=NULL, 
                      order=TRUE, nfilter=0, filtfacts=NULL,
                      returnStat=FALSE, horizontal=F, hline=NULL,
                      ...) {
  # build list to plot
  if (!is.vector(fact)) {
    if (!is.list(num)) {
      stop('if arg fact not supplied arg num must be list');
    } else {
      ltoplot = num;
    }
  } else {
    ltoplot = split(num, fact);
  }
  # build names
  ll = sapply(ltoplot, length);
  if (!is.character(names)) {
    names = paste0(names(ll),'(',ll,')');
  } else {
    names = paste0(names,'(',ll,')');
  }
  # filter list components with too few data points
  nkeep = sapply(ltoplot, length) >= nfilter;
  ltoplot = ltoplot[nkeep];
  names = names[nkeep];
  
  # filter unwanted list elements if indicated
  if (!is.null(filtfacts)) {
    if (is.character(filtfacts)) {
      fremove = na.omit(match(filtfacts, names(ltoplot)));
    }
    ltoplot = ltoplot[-fremove];
    names = names[-fremove];
  }
  ...
  if (is.null(col)) {
    ocolwasnull = TRUE;
    col = sample(blues9, length(ltoplot), replace=T);
  } else {
    ocolwasnull = FALSE;
  }
  
  # order by median if indicated
  if (order) {
    ord = order(sapply(ltoplot, median, na.rm=T));
    ltoplot = ltoplot[ord];
    names = names[ord];
    col = col[ord]
    if (ocolwasnull) {
  ...
  # compute stat and build main title
  kwt = kruskal.test(ltoplot);
  if (!is.character(main)) {
    main = paste0(deparse(substitute(num)),'\np=',signif(kwt$p.value, 2));
  } else {
    main = paste0(main,'\np=',signif(kwt$p.value, 2));
  }
  # plot
  
  boxplot(ltoplot,xlab='',ylab='',names=names,
          col=col,main=main,notch=T,varwidth=T,horizontal=horizontal,
          ...);
  if (!is.numeric(hline)) {
    if (horizontal) {
      abline(v=median(unlist(ltoplot), na.rm=T), col='grey');
    } else {
      abline(h=median(unlist(ltoplot), na.rm=T), col='grey');
    }
  } else {
    if (horizontal) {
      abline(v=hline, col='grey');
    } else {
      abline(h=hline, col='grey');
    }
  }
  if (returnStat) {return(kwt)}
}
...
.parseGOhitGenesAndTermHeatmap = function (GOresults, geneCol, geneDelim, termCol, originalAbGenes=NULL, numberTerms=T, ...) {
	GOparsed  = .parseGOhitGenes(GOresults, geneCol, geneDelim, termCol, originalAbGenes);
	termTable = .buildGeneTermTable(GOparsed);
	if (numberTerms) {
	  names(termTable) = paste0(1:ncol(termTable), '-', names(termTable));
	}
	termHeatmap = heatmap(1-cor(termTable), symm=T, scale='none', ...);
	return(list(parsed=GOparsed, termTable=termTable, heatmap=termHeatmap));
}


# inlist is a named list of named vectors, e.g...
#
# > lapply(head(dmrsDistToGenes), head)
# $`scaffold_1314:57386-58160`
# ubxn4 
    # 0 

# $`scaffold_1837:20196-20604`
# LOC102305293 
           # 0 

# $`scaffold_153:484022-485147`
 # adrm1   aff2  ap1ar ccdc37 commd5 commd7 
 # 82806 428516 826108 837863 748230 199163 

# $`scaffold_470:330484-331963`
       # gtdc1 LOC102295551 LOC102295742 LOC102296050 LOC102296369 LOC102296973 
        # 1808       328815       245929       216251        37508        32190 

# $`scaffold_760:111200-112827`
       # apbb3       cystm1 LOC102289896 LOC102290401 LOC102291435 LOC102291728 
       # 90343        85823        73644        61633        45476        44836 

# $`scaffold_14:162561-162989`
 # ablim3   acsl4  adam19 afap1l1   anxa6   apool 
 # 938622 1994401  699457  985382  136300 2591476 
 
# names of inlist elements cannot contain "." 
 # pretty slow.....
.flipListNamesAndElements = function (inlist) {
	x = unlist(inlist);
	xnames = strsplit(names(x), '.', fixed=T);
	outlist = list();
	for (i in 1:length(x)) {
		name1 = xnames[[i]][1];
		name2 = xnames[[i]][2];
		outlist[[name2]] = c(outlist[[name2]], x[i]);
		names(outlist[[name2]])[length(outlist[[name2]])] = name1;
	}
	return(outlist);
}


.checkKeywordEnrichment_genesNNdmr = function (genesNNdmr, string, dist, stringCol, distCol) {
  desc = genesNNdmr[, stringCol];
  dists = genesNNdmr[, distCol];
  total = nrow(genesNNdmr);
  dist_test = dists <= dist;
  string_test = grepl(string, desc, ignore.case=TRUE);
  yy = sum(dist_test & string_test);
  yn = sum(dist_test) - yy;
  ny = sum(string_test) - yy;
  nn = total - yy - yn - ny;
  testmat = matrix(c(yy, ny, (yy+ny),
                     yn, nn, (yn+nn),
                     (yy+yn), (ny+nn), (yy+yn+ny+nn)), 
                   ncol=3, 
                   dimnames=list(c(paste0('dist_',dist,'_',c('y','n')),''), 
                                 c(paste0(string,'_',c('y','n')),'')));
  return(list(testmat, 
              fisher.test(testmat[-3,-3]),
              genesNNdmr[string_test, ]));
}



.checkKeywordEnrichment_inDfCol = function (df, string, stringCol, rowsToTest, ...) {
  total = nrow(df);
  if (is.numeric(rowsToTest)) {
    rowsToTest = (1:total) %in% rowsToTest;
  }
  desc = df[, stringCol];
  string_test = grepl(string, desc, ...);
  yy = sum(rowsToTest & string_test);
  yn = sum(rowsToTest) - yy;
  ny = sum(string_test) - yy;
  nn = total - yy - yn - ny;
  testmat = matrix(c(yy, ny, (yy+ny),
                     yn, nn, (yn+nn),
                     (yy+yn), (ny+nn), (yy+yn+ny+nn)), 
                   ncol=3, 
                   dimnames=list(c(paste0(deparse(substitute(rowsToTest)),c('y','n')),''), 
                                 c(paste0(string,'_',c('y','n')),'')));
  return(list(testmat, 
              fisher.test(testmat[-3,-3]),
              df[string_test, ]));
...
.checkGeneListEnrichmentListList = function (list1, list2, refset, thresh=.05, alt='two.sided') {
  x = lapply(list1, 
             function(f) .checkGeneListEnrichmentList(f, list2, refset,
                                                      thresh=thresh, alt=alt, order=F));
  pvaldf = as.data.frame(sapply(x, function(f) f$pvals$pval));
  rownames(pvaldf) = names(list2);
  ordf = as.data.frame(sapply(x, function(f) f$pvals$ratio));
  rownames(ordf) = names(list2);
  
  nums_pcts = lapply(x, .getCountsAndPctsFrom_checkGeneListEnrichmentList_results);
  counts = as.data.frame(sapply(nums_pcts, function(f) f$num));
  rownames(counts) = names(list2);
  pcts = as.data.frame(sapply(nums_pcts, function(f) f$pct));
  rownames(pcts) = names(list2);
  
  return(list(full=x, pvals=pvaldf, ratios=ordf, counts=counts, pcts=pcts))
}

.getCountsAndPctsFrom_checkGeneListEnrichmentList_results = function (checkGeneListEnrichmentList_results) {
  x = checkGeneListEnrichmentList_results$details;
  counts = sapply(x, function(f) f[[1]][1,1]);
  totals = sapply(x, function(f) f[[1]][1,3]);
  return(data.frame(num=counts, pct=counts/totals))
}

.findOverlapsFullInfo = function (gr1, gr2, ...) {
  ov = as.data.frame(suppressWarnings(findOverlaps(gr1, gr2, ...)));
  gr1names = names(gr1); names(gr1) <- NULL; 
  gr2names = names(gr2); names(gr2) <- NULL; 
  gr1hitsGR = gr1[ov$queryHits];
  gr2hitsGR = gr2[ov$subjectHits];
  outdf = as.data.frame(cbind(queryname=gr1names[ov$queryHits],
                              as.data.frame(gr1hitsGR),
                              subjectname=gr2names[ov$subjectHits],
                              as.data.frame(gr2hitsGR)));
  names(gr1hitsGR) = gr1names[ov$queryHits];
  names(gr2hitsGR) = gr2names[ov$subjectHits];
  return(list(gr1hitsGR=gr1hitsGR,
              gr2hitsGR=gr2hitsGR,
              df=outdf));
}

.addDmrDistInfoToExpression = function (distlist, resdf) {
	.get_len_num0_mean = function (vec) {
		return(c(length(vec), sum(vec==0), mean(vec)));
	}
	.prepdmrs = function (thisdmrs, dmrcols1, dmrcols1names) {
		newdmrs = as.data.frame(cbind(matrix(apply(thisdmrs[, dmrcols1], 2, mean), ncol=4),
		                              all.mean=mean(apply(thisdmrs[,names(thisdmrs) %in% c('group1.mean','group2.mean')], 1, mean)),
		                              areaStat=mean(thisdmrs$areaStat),
		                              areaStat.abs=mean(abs(thisdmrs$areaStat)),
		                              meanDiff=mean(thisdmrs$meanDiff),
		                              meanDiff.abs=mean(abs(thisdmrs$meanDiff)),
		                              fc=mean(log2(thisdmrs$group2.mean/thisdmrs$group1.mean)),
		                              fc.abs=abs(mean(log2(thisdmrs$group2.mean/thisdmrs$group1.mean)))));
		names(newdmrs)[1:length(dmrcols1names)] = dmrcols1names;
		return(newdmrs);
	}
	
	
	distlist = distlist[sapply(distlist,length) > 0];
	genecheck = names(distlist) %in% rownames(resdf);
	if (!all(genecheck)) {
		distlist = distlist[genecheck];
		missinggenes = names(distlist)[!genecheck]
		warning(paste0(length(missinggenes),
		               ' genes missing from expression data frame: ',
		               paste0(missinggenes,collapse=',')));
	}
	
	distlistdf = as.data.frame(t(sapply(distlist, .get_len_num0_mean)));
	resdf = as.data.frame(cbind(resdf, matrix(nrow=nrow(resdf),ncol=3)));
	rownums = match(rownames(distlistdf), rownames(resdf));
	colnums = ((ncol(resdf))-2):ncol(resdf)
	resdf[rownums, colnums] = distlistdf;
	names(resdf)[colnums] = c('dmr.num','dmr.numOV','dmr.meanDist');
	resdf$dmr.num[is.na(resdf$dmr.num)] = 0;
	resdf$dmr.numOV[is.na(resdf$dmr.numOV)] = 0;
	
	dmrlist = lapply(distlist, 
	                 function(f) dmrs[match(names(f), rownames(dmrs)), 7:ncol(dmrs)]);
	dmrcols1 = c('n','width','invdensity','GC');
	dmrsreduced = do.call(rbind, 
						  lapply(dmrlist, 
						  		 function(f) .prepdmrs(f, 
						  		 						match(dmrcols1, names(dmrlist[[1]])), 
						  		 					    dmrcols1)));
	
	resdf = as.data.frame(cbind(resdf, matrix(nrow=nrow(resdf),ncol=11)));
	colnums = ((ncol(resdf))-10):ncol(resdf)
	resdf[rownums, colnums] = dmrsreduced;
	names(resdf)[colnums] = names(dmrsreduced);

	resdf = as.data.frame(cbind(resdf, 
	                      fc.anti=(resdf$log2FoldChange / resdf$fc) < 0,
	                      log2FoldChange.abs = abs(resdf$log2FoldChange)))
		
	return(resdf);
}


.dmrsDistToGenesStats = function (dmrsDistToGenes, test_dists) {
  # make thresholded distance lists
  dmrsDistToGenes_thresh = list();
  for (d in test_dists) {
    cat(as.character(d),'...')
    dmrsDistToGenes_thresh[[as.character(d)]] = lapply(dmrsDistToGenes, function(f) f[f <= d]);
  }
  # make data frame where rows are dmrs, columns are window sizes, values are numbers of genes
  countDf = as.data.frame(sapply(dmrsDistToGenes_thresh[[1]], length));
  # make data frame where rows are dmrs, columns are window sizes, values are distances to genes
  distDf = as.data.frame(sapply(dmrsDistToGenes_thresh[[1]], mean));
  #
  for (d in 2:length(dmrsDistToGenes_thresh)) {
    countDf = as.data.frame(cbind(countDf,
                                  sapply(dmrsDistToGenes_thresh[[d]], length) ));
    distDf = as.data.frame(cbind(distDf,
                                 sapply(dmrsDistToGenes_thresh[[d]], mean) ));
  }
  names(countDf) = names(dmrsDistToGenes_thresh);
  names(distDf) = names(dmrsDistToGenes_thresh);
  #
  summaryDf = as.data.frame(cbind(count=apply(countDf, 2, mean, na.rm=T),
                                  dist=apply(distDf, 2, mean, na.rm=T)));
  return(list(threshlist=dmrsDistToGenes_thresh,
              countDf=countDf,
              distDf=distDf,
              summaryDf=summaryDf));
}
...
.reduceKeepMcols = function (gr, matchOrder=T) {
  x = reduce(gr, with.revmap=T);
  inds = sapply(mcols(x)[,1], function(f) f[1]);
  mcols(x) = mcols(gr)[inds, ];
  names(x) = names(gr)[inds];
  if (matchOrder) { x = x[order(inds)] }
  return(x);
}

.get_res_rowsWithDmrs = function (res, dmrvec, dmrIDcol) {
  return(as.data.frame(do.call(rbind,
                               lapply(as.list(dmrvec), 
                                      function(f) res[grepl(f,res[,dmrIDcol]),]))))
}

# fglist is a named list of character vectors
# ids is character vector holding ids that are spread across vectors in fglist
.make_factor_from_fglist = function (fglist, ids, fginds) {
  if (is.character(fginds)) { fginds = match(fginds, names(fglist)) }
  xlist = fglist[fginds];
  factvec = sapply(as.list(ids), 
                   function (thisid) 
                     paste0(names(xlist)[sapply(xlist, 
                                                function(f) 
                                                  any(f==thisid))], 
                            collapse=':'));
  return(factvec)
...
.make_factor_from_names_of_logical_columns = function (df, cols) {
  df = df[, cols];
  x = apply(df, 1, 
            function(f) 
              paste0(names(df)[as.logical(f)], collapse=':'));
  return(x)
}



.computeAndPlotDfCors = function(df1, df2=NULL, 
                                 col=blueWhiteRed(50), cex.text=.5, main='', 
                                 thresh=(.05/(ncol(df2)*ncol(df1))), 
                                 reorderdf2=NULL, suppressPlot=F, ...) {
  if (!is.null(reorderdf2)) {
    if (is.vector(reorderdf2) & is.numeric(reorderdf2) & length(reorderdf2)==ncol(df2)) {
      cat('reordering df2...\n');
      df2 = df2[, reorderdf2];
    } else {
      warning(paste('Ignoring "reorderdf2", either is not vector, not numeric, or not same length df2'));
    }
  }
  if (is.null(df2)) {
    xcor = df1;
    xcorP = WGCNA::corPvalueStudent(xcor, nrow(df1));
    ylabs = names(df1);
    ysyms = names(df1);
  } else {
    xcor = cor(df2, df1, use='p', ...);
    xcorP = WGCNA::corPvalueStudent(xcor, nrow(df1));
    ylabs = names(df2);
    ysyms = names(df2);
  }
  ...
  if (suppressPlot) {
    return(list(cor=xcor, pval=xcorP));
  } else {
    textMat = paste(signif(xcor, 2), '\n(', signif(xcorP, 1), ')', sep='');
    dim(textMat) = dim(xcor);
    textMat[xcorP>thresh] = '';
    par( mar = c(8, 9, 3, 3) );
    WGCNA::labeledHeatmap(Matrix=xcor, xLabels=names(df1), yLabels=ylabs, ySymbols=ysyms, colorLabels=F, colors=col, textMatrix=textMat, setStdMargins=F, cex.text=cex.text, zlim=c(-1, 1), main=main, ...); 
    return(list(cor=xcor, pval=xcorP));
  }
}

.limit_checkGeneListEnrichmentListList = function (checkGeneListEnrichmentListListOutput, 
                                                   plim=NULL) {
  xp = checkGeneListEnrichmentListListOutput$pvals;
  if (is.numeric(plim)) {
    plim = plim;
  } else if (is.character(plim)) {
    if (plim=='bonf') {
      xp = xp*(nrow(xp)*ncol(xp));
    } else if (plim=='BH') {
      xp = as.data.frame(apply(xp, 2, function(f) p.adjust(f,method='BH')));
    } else if (plim=='BHstrict') {
      xp = as.data.frame(apply(xp, 2, function(f) p.adjust(f,method='BH',n=(nrow(xp)*ncol(xp)))));
    } else {
      stop('arg plim must be numeric or one of "bonf", "BH", "BHstrict"');
    }
    plim = .05;
  } else {
    warning('arg plim is not numeric or one of "bonf", "BH", "BHstrict"... using .05');
    plim = .05;
  }
  goodrows = apply(xp,1,function(f)any(f <= plim));
  goodcols = apply(xp,2,function(f)any(f <= plim));
  return(list(pvals=xp[goodrows,goodcols],
              ratios=checkGeneListEnrichmentListListOutput$ratios[goodrows,goodcols],
              counts=checkGeneListEnrichmentListListOutput$counts[goodrows,goodcols],
              pcts=checkGeneListEnrichmentListListOutput$pcts[goodrows,goodcols]))
}


.buildOverlapTableFromListOfVecs = function (inlist) {
  df = as.data.frame(matrix(ncol=length(inlist),
                            nrow=length(inlist),
                            dimnames=list(names(inlist),names(inlist))));
  xcounts = lapply(as.list(1:length(inlist)), 
                   function(ff) 
                     sapply(inlist[-ff], 
                            function(f) sum(f %in% inlist[[ff]])));
  for (i in 1:length(xcounts)) {
    df[match(names(xcounts[[i]]), rownames(df)), i] = xcounts[[i]];
  }
  return(df);
}


.buildNotRaggedDfFromVecList = function (veclist) {
  allids = unique(unlist(veclist));
  x = as.data.frame(matrix(ncol=length(veclist),
                           nrow=length(allids),
                           dimnames=list(allids,names(veclist))));
  for (i in 1:length(veclist)) {
    xrows = match(veclist[[i]], rownames(x));
    x[xrows,i] = TRUE;
  }
  x[which(is.na(x),arr.ind=T)] = FALSE;
  xnum = as.data.frame(apply(x,2,as.numeric));
  rownames(xnum) = rownames(x);
  return(list(log=x,num=xnum))
}

.filterVecListToUniques = function (veclist) {
  x = veclist;
  for (i in 1:length(veclist)) {
    x[[i]] = veclist[[i]][!(veclist[[i]] %in% unique(unlist(veclist[-i])))]
  }
  return(x)
}

.writeGenesForKEGGMapping = function (enrichKEGGout, geneCol=8, geneSep='/', backCol='blue', textCol='yellow') {
  x = enrichKEGGout
  for (i in 1:nrow(x)) {
    k = unlist(strsplit(x[i,geneCol], geneSep));
    write.table(paste0(k, ' ', 
                       rep(backCol,length(k)), ',',
                       rep(textCol,length(k))),
                quote=F, row.names=F, col.names=F,
                file=paste0(rownames(x)[i], '_', x[i,'Description'],'.txt'));
  }
}

.countGeneHitsInParsedTermTable = function (termTable, genes) {
  x = termTable[rownames(termTable) %in% genes, ];
  xT = apply(x, 2, function(f) any(f>0));
  return(sort(apply(x[,xT],1,sum),decreasing=T))
}

.subset_resByGenesInThreshedVec = function (thisres, vec, thresh) {
  genes = names(vec)[vec >= thresh];
  x = subset(thisres, geneSym %in% genes);
  return(x[match(genes, x$geneSym), ]);
}

.subset_resForGenesInNumberOfTerms = function (thisres, genes, termTable, thresh) {
  gvec = .countGeneHitsInParsedTermTable(termTable, genes);
  sres = .subset_resByGenesInThreshedVec(thisres, gvec, thresh);
  return(as.data.frame(cbind(sres, term.num=gvec[match(sres$geneSym, names(gvec))])));
}

.makeDfFromListOfNamedVecs = function (listOfVecs) {
  numvecs = length(listOfVecs);
  vecnames = names(listOfVecs);
  elementnames = sort(unique(unlist(lapply(listOfVecs,names))));
  numelements = length(elementnames);
  
  df = as.data.frame(matrix(rep(NA, (numelements*numvecs)), 
                            ncol=numvecs,
                            dimnames=list(elementnames,vecnames)));
  
  for (v in 1:numvecs) {
    thisvec = listOfVecs[[v]];
    if (length(thisvec)==0) { thisvec = NA }
    df[match(names(thisvec), rownames(df)), v] = thisvec;
  }
...
.getTfHitsForGeneDMR = function (geneSym, 
                                 dmrTfHitDf, 
                                 dmrFeatOvDf, 
                                 dmrTfHitDf_idColName='subjectname', 
                                 dmrFeatOvDf_idColName='mcols.geneSym'
                                 ) {
  dmrIDs = dmrFeatOvDf[dmrFeatOvDf[,dmrFeatOvDf_idColName]==geneSym, 'queryHits'];
  return(dmrTfHitDf[dmrTfHitDf[,dmrTfHitDf_idColName] %in% dmrIDs, ])
}